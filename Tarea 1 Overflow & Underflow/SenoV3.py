...
def seno(x):
    ...
    amenos = 0
    n = 1
    while abs(a / sumamenos) > (10**(-8)):    #tamano del error absoluto 
        amenos = a
        sumamenos = suma
        n = n + 1
...
for i in range(h):
    particion[i] = (2*pi*(1+i)) / h #Rango de la particion 
...
df_resultados.to_excel('resultados_seno.xlsx', index=False) #Paso de listas de python a excel para despue pasarlas a tablas en LateX
