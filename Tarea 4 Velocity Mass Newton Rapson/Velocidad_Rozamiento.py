...
def Vo_Xdeseada(t, x, y, angulo, f, w, Xdeseada, err, max_iter, i):
    Vo_min = 20
    Vo_max = 200
    Vo_deseada = 0  
    
    iter_count = 0
    while (Vo_max - Vo_min) / 2 > err and iter_count < max_iter:
        Vo_deseada = (Vo_max + Vo_min) / 2  # Punto medio del intervalo
        f(t, x, y, Vo_deseada, angulo, i)  
        if distancia_max(t, x, y) == Xdeseada:
            return Vo_deseada, iter_count
        elif Xdeseada - distancia_max(t, x, y) < 0:
            Vo_max = Vo_deseada  # La raiz esta en el subintervalo [a, c]
        else:
            Vo_min = Vo_deseada  # La raiz esta en el subintervalo [c, b]
        iter_count += 1

    return (Vo_min + Vo_max) / 2
...
for i in range(3):  
    ...
    #Velocida_champeon[i] = Vo_Xdeseada(tiempo, x, y, angulo, f, distancia_max, xf, err, 60, i) 
    Velocida_champeon[i] = Vo_Xdeseada(tiempo, x, y, angulo, f, distancia_max, xf, err, 60, i) 
    ...
    x_max[i] = distancia_max(tiempo, x, y)
